<?php
    ...
    if(isset($_POST['btnCrear'])){
        ...
        if($exito==true){
            ...
            (new Tiendas)->setNombre($nombre)->setVideojuego_id($vid)->update($id);
...
<body style="background-color:#ffebcd">
    ...
    <div class="container mt-2">
        <div class="bg-success p-4 text-white rounded shadow-lg mx-auto" style="width:48rem">
            <form name="videojuego" action="<?php echo $_SERVER['PHP_SELF']."?id=$id" ?>" method="POST">

                <div class="mb-3">
                    <label for="n" class="form-label">Nombre de la tienda</label>
                    <input type="text" class="form-control" id="n" placeholder="Nombre" name="nombre" value="<?php echo $laTienda->nombre ?>">
                    ...
                    <select class="form-select" name="videojuego_id" id="v">
                        <?php
                            ...
                            foreach($videojuegos->fetchAll(PDO::FETCH_OBJ) as $item){
                                if($item->id == $laTienda->videojuego_id){
                ...
                <div>
                    <button type="submit" name="btnCrear" class="btn btn-primary"><i class="fas fa-save"></i> Actualizar</button>
